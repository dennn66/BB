...
void Dongle::sendKeyToDongle(int condition_index){
    ...
    if(currentl2w == NULL)return;
     ...
     if(currentl2w->getCurrentSettings()->condition[condition_index]->FSet){
        ...
        if(
            (((currentl2w->getCurrentSettings()->condition[condition_index]->getTargetTypeBinaryState() & 0b00001111 ) != 0b00001111) || // Target Condition
...
void Dongle::setDeviceState(unsigned char state){
